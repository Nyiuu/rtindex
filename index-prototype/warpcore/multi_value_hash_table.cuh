...
public:
    ...
    */
    ...
    /*! \brief joins the relation stored inside the table with another given relation
     * \info this method has a dry-run mode where it only calculates the needed array sizes in case no memory (aka \c nullptr ) is provided
     * \tparam ValueS value type of the second relation
     * \param[in] keys_in keys of the second relation
     * \param[in] values_in values the second relation
     * \param[in] num_in number of rows inside the second relation
     * \param[out] result_out array of generated/joined tuples of type \c thrust::tuple<key_type,value_type,ValueS>
     * \param[out] num_out total number of emitted tuples
     * \param[in] stream CUDA stream in which this operation is executed in
     * \param[in] probing_length maximum number of probing attempts
     */
    ...
    void natural_join(
        const key_type * const keys_in,
        const ValueS * const values_in,
        const index_type num_in,
        thrust::tuple<key_type, value_type, ValueS> * const result_out,
        index_type& num_out,
        const cudaStream_t stream = 0,
        const index_t probing_length = defaults::probing_length()) const noexcept
    {
        using result_t = std::tuple<key_type, value_type, ValueS>;

        index_type * count = temp_.get();
        cudaMemsetAsync(count, 0, sizeof(index_type), stream);
        ...
        ([=, *this] DEVICEQUALIFIER
        {
            ...
            if(gid < num_in)
            {
                ...
                const auto value_s = values_in[gid];
                index_type num_values_out;

                for_each([=] DEVICEQUALIFIER (
                    const key_type /* key */,
                    const value_type& value_r,
                    const index_type /* num */)
                    {
                        const auto offset = atomicAggInc(count);

                        if(result_out != nullptr)
                        {
                            result_out[offset] =
                                thrust::make_tuple(key, value_r, value_s);
                        }
                    },
                    key,
                    num_values_out,
        ...
        cudaMemcpyAsync(&num_out, count, sizeof(index_type), D2H, stream);
