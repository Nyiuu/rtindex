...
optix_pipeline::optix_pipeline(optix_wrapper* optix, bool verbose) : optix{optix} {
    if (verbose) {std::cerr << "#rtx: creating raygen programs ..." << std::endl;}
    create_raygen_programs();

    if (verbose) {std::cerr << "#rtx: creating miss programs ..." << std::endl;}
    create_miss_programs();

    if (verbose) {std::cerr << "#rtx: creating hitgroup programs ..." << std::endl;}
    create_hitgroup_programs();

    if (verbose) {std::cerr << "#rtx: setting up optix pipeline ..." << std::endl;}
    assemble_pipeline();

    if (verbose) {std::cerr << "#rtx: building SBT ..." << std::endl;}
    build_sbt();

    if (verbose) {std::cerr << "#rtx: pipeline all set up ..." << std::endl;}
}
